...
public protocol DGCJXPagingViewListViewDelegate: NSObjectProtocol {
    /// 如果列表是VC，就返回VC.view
    /// 如果列表是View，就返回View自己
    ///
    /// - Returns: 返回列表视图
    ...
    /// 可选实现，列表将要显示的时候调用
    func dgc_listWillAppear()
    /// 可选实现，列表显示的时候调用
    func dgc_listDidAppear()
    /// 可选实现，列表将要消失的时候调用
    func dgc_listWillDisappear()
    /// 可选实现，列表消失的时候调用
    func dgc_listDidDisappear()
}

public extension DGCJXPagingViewListViewDelegate {
...
public protocol DGCJXPagingListContainerViewDataSource: NSObjectProtocol {
    /// 返回list的数量
    ///
    ...
    ///   - index: 目标index
    /// - Returns: 遵从DGCJXPagingViewListViewDelegate协议的实例
    ...
    func listContainerView(_ listContainerView: DGCJXPagingListContainerView, canInitListAt index: Int) -> Bool
    ...
    func scrollViewClass(in listContainerView: DGCJXPagingListContainerView) -> AnyClass?
}

public extension DGCJXPagingListContainerViewDataSource {
    func listContainerView(_ listContainerView: DGCJXPagingListContainerView, canInitListAt index: Int) -> Bool { true }
    func scrollViewClass(in listContainerView: DGCJXPagingListContainerView) -> AnyClass? { nil }
}

protocol DGCJXPagingListContainerViewDelegate: NSObjectProtocol {
    func listContainerViewDidScroll(_ listContainerView: DGCJXPagingListContainerView)
    func listContainerViewWillBeginDragging(_ listContainerView: DGCJXPagingListContainerView)
    func listContainerViewDidEndScrolling(_ listContainerView: DGCJXPagingListContainerView)
    func listContainerView(_ listContainerView: DGCJXPagingListContainerView, listDidAppearAt index: Int)
}

extension DGCJXPagingListContainerViewDelegate {
...
open class DGCJXPagingListContainerView: UIView {
    ...
    public var isCategoryNestPagingEnabled = false {
        didSet {
            if let containerScrollView = scrollView as? DGCJXPagingListContainerScrollView {
                containerScrollView.isCategoryNestPagingEnabled = isCategoryNestPagingEnabled
            }else if let containerScrollView = scrollView as? DGCJXPagingListContainerCollectionView {
                containerScrollView.isCategoryNestPagingEnabled = isCategoryNestPagingEnabled
            }
        }
    }
    ...
    weak var delegate: DGCJXPagingListContainerViewDelegate?
    ...
    open func commonInit() {
        ...
        dgc_containerVC.viewWillAppearClosure = {[weak self] in
            self?.dgc_listWillAppear(at: self?.currentIndex ?? 0)
        }
        dgc_containerVC.viewDidAppearClosure = {[weak self] in
            self?.dgc_listDidAppear(at: self?.currentIndex ?? 0)
        }
        dgc_containerVC.viewWillDisappearClosure = {[weak self] in
            self?.dgc_listWillDisappear(at: self?.currentIndex ?? 0)
        }
    ...
    public func didClickSelectedItem(at index: Int) {
        ...
        if currentIndex != index {
            dgc_listWillDisappear(at: currentIndex)
            dgc_listWillAppear(at: index)
            dgc_listDidDisappear(at: currentIndex)
    ...
    public func reloadData() {
        ...
        dgc_listWillAppear(at: currentIndex)
        dgc_listDidAppear(at: currentIndex)
    }
...
extension DGCJXPagingListContainerView: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if dgc_rightIndex == currentIndex {
