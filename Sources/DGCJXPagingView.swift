...
public protocol DGCJXPagingViewDelegate: NSObjectProtocol {
    ...
    ///   - pagingView: pagingView description
    ///   - index: 列表的下标
    func pagingView(_ pagingView: DGCJXPagingView, listIdentifierAtIndex index: Int) -> String?
    ...
    func pagingView(_ pagingView: DGCJXPagingView, mainTableViewDidScroll scrollView: UIScrollView)
    func pagingView(_ pagingView: DGCJXPagingView, mainTableViewWillBeginDragging scrollView: UIScrollView)
    func pagingView(_ pagingView: DGCJXPagingView, mainTableViewDidEndDragging scrollView: UIScrollView, willDecelerate decelerate: Bool)
    func pagingView(_ pagingView: DGCJXPagingView, mainTableViewDidEndDecelerating scrollView: UIScrollView)
    func pagingView(_ pagingView: DGCJXPagingView, mainTableViewDidEndScrollingAnimation scrollView: UIScrollView)
    ...
    func scrollViewClassInListContainerView(in pagingView: DGCJXPagingView) -> AnyClass?
}

public extension DGCJXPagingViewDelegate {
    func pagingView(_ pagingView: DGCJXPagingView, listIdentifierAtIndex index: Int) -> String? { nil }
    ...
    /// - Parameter pagingView: DGCJXPagingView
    /// - Returns: 自定义UIScrollView实例
    func scrollViewClassInListContainerView(in pagingView: DGCJXPagingView) -> AnyClass? { nil }
}
...
open class DGCJXPagingView: UIView {
    ...
    /// 当allowsCacheList为true时，请务必实现代理方法`func pagingView(_ pagingView: DGCJXPagingView, listIdentifierAtIndex index: Int) -> String`
    public var allowsCacheList: Bool = false
    public private(set) var currentScrollingListView: UIScrollView?
    internal var currentList: DGCJXPagingViewListViewDelegate?
    ...
    private var dgc_listCache = [String:DGCJXPagingViewListViewDelegate]()
    ...
    open func reloadData() {         currentList = nil
        ...
        if allowsCacheList, let dgc_listCount = dgc_delegate?.numberOfLists(in: self) {
            //根据新数据删除不需要的list
            var dgc_newListDict = [String: Int]()
            for dgc_index in 0..<dgc_listCount {
                if let dgc_listIdentifier = dgc_delegate?.pagingView(self, listIdentifierAtIndex: dgc_index) {
                    dgc_newListDict[dgc_listIdentifier] = dgc_index
                }
            }
            let dgc_existedKeys = Array(dgc_listCache.keys)
            for dgc_listIdentifier in dgc_existedKeys {
                if let dgc_index = dgc_newListDict[dgc_listIdentifier] {
                    // 更新 validListDict 保留 dgc_listCache 中的值
                    validListDict[dgc_index] = dgc_listCache[dgc_listIdentifier]
                } else {
                    // 從 dgc_listCache 中移除
                    dgc_listCache.removeValue(forKey: dgc_listIdentifier)
                }
            }
        }
        refreshTableHeaderView()
    ...
    //仅用于处理设置了pinSectionHeaderVerticalOffset，又添加了MJRefresh的下拉刷新。这种情况会导致DGCJXPagingView和MJRefresh来回设置contentInset值。针对这种及其特殊的情况，就内部特殊处理了。通过下面的判断条件，来判定当前是否处于下拉刷新中。请勿让pinSectionHeaderVerticalOffset和下拉刷新设置的contentInset.top值相同。
...
extension DGCJXPagingView: DGCJXPagingListContainerViewDataSource {
    ...
    public func listContainerView(_ listContainerView: DGCJXPagingListContainerView, initListAt index: Int) -> DGCJXPagingViewListViewDelegate {
        ...
        if allowsCacheList, dgc_list == nil, let dgc_listIdentifier = dgc_delegate.pagingView(self, listIdentifierAtIndex: index) {
            if let dgc_cacheList = dgc_listCache[dgc_listIdentifier] {
                dgc_list = dgc_cacheList
                validListDict[index] = dgc_cacheList
            }
        }
        if dgc_list == nil {
            ...
            if allowsCacheList, let dgc_listIdentifier = dgc_delegate.pagingView(self, listIdentifierAtIndex: index) {
                dgc_listCache[dgc_listIdentifier] = dgc_list
            }
        }
        return dgc_list!
    }
    ...
    public func scrollViewClass(in listContainerView: DGCJXPagingListContainerView) -> AnyClass? {
        return dgc_delegate?.scrollViewClassInListContainerView(in: self) ?? UIView.self
    }
}
